...
async function listWorkflowsFor(workspaceId: string): Promise<WorkflowListEntry[]> {
  const supabase = getSupabaseAdmin();
  if (!supabase) return [];
  const { data, error } = await supabase
    .from("workspace_workflows")
    .select("id, name, description, enabled, trigger, steps, updated_at")
  ...
  return (data as Array<Record<string, unknown>>).map((row) => ({
    ...
    triggerType: String(((row.trigger as Record<string, unknown> | null)?.type as string | undefined) ?? "—"),
    stepsCount: Array.isArray(row.steps) ? (row.steps as unknown[]).length : 0,
    updatedAt: String(row.updated_at),
  }));
}

export default async function WorkflowsListPage({ params }: PageProps) {
  const { workspaceSlug } = await params;
  const user = await requireAuthenticatedUser(`/workspaces/${workspaceSlug}/crm/workflows`);
  ...
  const [teamChatChannels, workflows] = await Promise.all([
    listWorkspaceChannelsForUser(workspaceSlug, user.id, user.isPlatformAdmin),
    listWorkflowsFor(snapshot.workspace.id),
  ]);
        ...
        <div style={headerWrapStyle}>
          <div style={headerInnerStyle}>
            <div>
              ...
              <p style={subtitleStyle}>Automatizaciones disparadas por eventos CRM.</p>
            </div>
            {canManage ? (
              <Link href={`/workspaces/${workspaceSlug}/crm/workflows/new`} style={primaryBtnStyle}>
                <Plus size={14} />
                Nuevo workflow
              </Link>
            ) : null}
          ...
          <div style={panelStyle}>
            {workflows.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {workflows.map((wf) => (
                  <Link key={wf.id} href={`/workspaces/${workspaceSlug}/crm/workflows/${wf.id}`} style={rowStyle}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <Zap size={16} color="var(--workspace-accent)" />
                      <div style={{ display: "flex", flexDirection: "column" }}>
                        <strong style={{ fontSize: 13 }}>{wf.name}</strong>
                        <span style={{ fontSize: 11, color: "var(--workspace-muted)" }}>
                          Trigger: {wf.triggerType} · {wf.stepsCount} pasos
                        </span>
                      </div>
                    </div>
                    <span
                      style={{
                        ...
                        color: wf.enabled ? "#047857" : "#374151",
                        fontWeight: 600,
                      }}
                    >
