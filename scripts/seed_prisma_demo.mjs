...
async function main() {
  const georgeEmail = "george@bbc.local";
  const mariaEmail = "maria@bbc.local";
  const carlosEmail = "carlos@bbc.local";
  const adminId = await ensureUser(georgeEmail, "admin");
  const operatorId = await ensureUser(mariaEmail, "operator");
  const viewerId = await ensureUser(carlosEmail, "viewer");
  ...
  await ensureMembership(demoWorkspace.id, viewerId, "viewer");
  ...
  const objectDefs = [
    ...
    {
      name: "Rate Offers",
      singularName: "Rate Offer",
      pluralName: "Rate Offers",
      description: "Pending and approved rate recommendations generated by cron analysis.",
      icon: "badge-percent",
      fields: [
        { name: "Offer name", key: "offer_name", type: "text", required: true },
        { name: "Company name", key: "company_name", type: "text", required: true },
        { name: "Suggested rate", key: "suggested_rate", type: "number", required: true },
        { name: "Exposure amount", key: "exposure_amount", type: "currency", required: true },
        { name: "Reason", key: "reason", type: "text" },
        { name: "Owner", key: "owner", type: "text" },
        {
          name: "Status",
          key: "status",
          type: "status",
          options: { values: ["awaiting_approval", "approved", "rejected"] },
      ...
      views: [
        {
          ...
          filters: { conditions: [{ field: "status", operator: "eq", value: "awaiting_approval" }] },
          sortBy: "offer_name",
          sortOrder: "asc",
          columns: ["offer_name", "company_name", "suggested_rate", "exposure_amount", "owner", "status"],
        },
      ],
      records: [
        {
          seedKey: "offer-grupo-norte-apr",
          data: {
            offer_name: "Oferta abril - Grupo Norte",
            company_name: "Grupo Norte",
            suggested_rate: 3.2,
            exposure_amount: "$850K MXN",
            reason: "Riesgo en bucket 61-90, se recomienda oferta con validación humana.",
            owner: "Rate Intelligence Agent",
            status: "awaiting_approval",
          },
        },
        {
          seedKey: "offer-retail-bajio-apr",
          data: {
            offer_name: "Oferta abril - Retail del Bajio",
            company_name: "Retail del Bajio",
            suggested_rate: 2.9,
            exposure_amount: "$420K MXN",
            reason: "Comportamiento estable, oportunidad de mejora táctica.",
            owner: "Rate Intelligence Agent",
            status: "awaiting_approval",
          },
        },
    ...
    cron_jobs: [
      {
        ...
        prompt: "Review receivables with follow_up or overdue status and log the queue summary with workspace {workspace_id}, today {today}, and since {last_run}.",
      },
      {
        schedule: "0 8 1 * *",
        prompt: "Generate monthly rate intelligence recommendations for pending approvals in Rate Offers and mention {workspace_id}, {today}, {last_run}.",
      },
    ],
    channel_config: {},
    memory_limit_mb: 384,
  ...
  await ensureActivity(monitorId, demoWorkspace.id, "cron.executed", "cron-rate-analysis", {
    title: "Rate intelligence mensual ejecutado",
    status: "success",
    generated_offers: 2,
    workspace_id: demoWorkspace.id,
  });

  await ensureActivity(monitorId, demoWorkspace.id, "rate_offer.generated", "rate-offer-generated", {
    offer: "Oferta abril - Grupo Norte",
    company: "Grupo Norte",
    status: "awaiting_approval",
    note: "Pendiente de aprobación por operador.",
  });
